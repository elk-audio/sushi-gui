...
class MainWindow(QMainWindow):
    ...
    def __init__(self, sushi_address: str) -> None:
        ...
        self.processor_notification_received.connect(
            self.process_processor_notification
        )
        self.parameter_notification_received.connect(
            self.process_parameter_notification
        )
        self.transport_notification_received.connect(
            self.process_transport_notification
        )
    ...
    def create_processor_on_track(
        self, plugin_info: sushi.ProcessorInfo, track_id: int
    ) -> None:
    ...
    def process_processor_notification(self, n) -> None:
        if n.action == 1:  # PROCESSOR_ADDED
            for t in self._controller.audio_graph.get_track_processors(
                n.parent_track.id
            ):
            ...
            if self.tracks[n.parent_track.id].processors[n.processor.id].to_be_moved:
                self.tracks[n.parent_track.id].processors[
                    n.processor.id
                ].to_be_moved = False
                return
    ...
    def process_parameter_notification(self, n) -> None:
        for id, track in self.tracks.items():
            if n.parameter.processor_id == id:
                ...
                track.processors[
                    n.parameter.processor_id
                ].handle_parameter_notification(n)
    ...
    def process_timing_notification(self, n=None) -> None:
        if n:
            timings = [n.main.average]
            timings.extend([t.average for t in n.threads])
            self.tpbar.set_cpu_value(timings)
