...
if TYPE_CHECKING:
    ...
    from sushi_gui.controller import Controller
...
class TransportBarWidget(QGroupBox):
    def __init__(self, parent: "MainWindow") -> None:
        ...
        self._tp_layout = QHBoxLayout(self)
        self._timings_layout = QHBoxLayout(self)
        self._main_layout.addLayout(self._tp_layout)
        self._main_layout.addLayout(self._timings_layout)
    ...
    def _create_widgets(self) -> None:
        ...
        self._tp_layout.addWidget(self._play_button)

        self._add_track_button = QPushButton("New Track", self)
        self._tp_layout.addWidget(self._add_track_button)

        self._cpu_meter = QLabel("Cpu: -", self)
        self._timings_layout.addWidget(self._cpu_meter)
...
class TrackWidget(QGroupBox):
    ...
    def _create_common_controls(self, track_info: sushi.TrackInfo) -> None:
        ...
        self._pan_gain = [
            PanGainWidget(self._id, "Main Bus", 0, 1, self._controller, self)
        ]
        ...
        for bus in range(1, track_info.buses):
            gain_id = self._controller.parameters.get_parameter_id(
                track_info.id, "gain_sub_" + str(bus)
            )
            pan_id = self._controller.parameters.get_parameter_id(
                track_info.id, "pan_sub_" + str(bus)
            )
            pan_gain = PanGainWidget(
                self._id, "Sub Bus " + str(bus), gain_id, pan_id, self._controller, self
            )
        ...
        self._mute_id = self._controller.parameters.get_parameter_id(
            track_info.id, "mute"
        )
        self._mute_button.setChecked(
            self._controller.parameters.get_parameter_value(self._id, self._mute_id)
            == 1
        )
...
class ProcessorWidget(QGroupBox):
    def __init__(
        self,
        controller: "Controller",
        processor_info: sushi.ProcessorInfo,
        track_id: int,
        parent: "MainWindow",
    ) -> None:
    ...
    def _create_common_controls(self, processor_info: sushi.ProcessorInfo) -> None:
        ...
        self._mute_button.setCheckable(True)
        self._mute_button.setChecked(
            self._controller.audio_graph.get_processor_bypass_state(self._id)
        )
        self._mute_button.setIcon(
            self.style().standardIcon(getattr(QStyle, "SP_MediaVolumeMuted"))
        )
        ...
        self._up_button.setIcon(
            self.style().standardIcon(getattr(QStyle, "SP_ArrowUp"))
        )
        ...
        if processor_info.program_count > 0:
            ...
            current_program = self._controller.programs.get_processor_current_program(
                self._id
            )
    ...
    def _connect_signals(self) -> None:
        ...
        self._program_selector.currentIndexChanged.connect(
            self.program_selector_changed
        )
    ...
    def handle_parameter_notification(self, notif: sushi.ParameterInfo) -> None:
        self._parameters[notif.parameter.parameter_id].set_slider_value(
            notif.normalized_value
        )
        self._parameters[notif.parameter.parameter_id].set_label_value(
            notif.formatted_value
        )
...
class ParameterWidget(QWidget):
    def __init__(
        self,
        parameter_info: sushi.ParameterInfo,
        processor_id: int,
        controller: "Controller",
        parent: "MainWindow",
    ) -> None:
        ...
        value = self._controller.parameters.get_parameter_value(
            self._processor_id, self._id
        )
        self.set_slider_value(value)
        txt_value = self._controller.parameters.get_parameter_value_as_string(
            self._processor_id, self._id
        )
    ...
    def value_changed(self) -> None:
        ...
        self._controller.parameters.set_parameter_value(
            self._processor_id, self._id, value
        )
...
class PropertyWidget(QWidget):
    def __init__(
        self,
        property_info: sushi.PropertyInfo,
        processor_id: int,
        controller: "Controller",
        parent: "MainWindow",
    ) -> None:
        ...
        value = self._controller.parameters.get_property_value(
            self._processor_id, self._id
        )
    ...
    def value_changed(self) -> None:
        ...
        self._controller.parameters.set_property_value(
            self._processor_id, self._id, value
        )
    ...
    def open_file_dialog(self) -> None:
        ...
        if dialog.exec_():
            ...
            self._controller.parameters.set_property_value(
                self._processor_id, self._id, filename
            )
...
class PanGainWidget(QWidget):
    def __init__(
        ...
        value = self._controller.parameters.get_parameter_value(
            self._processor_id, pan_id
        )
        self.set_pan_slider(value)
        txt_value = self._controller.parameters.get_parameter_value_as_string(
            self._processor_id, self.pan_id
        )
        self.set_pan_label(txt_value)

        value = self._controller.parameters.get_parameter_value(
            self._processor_id, gain_id
        )
        self.set_gain_slider(value)
        txt_value = self._controller.parameters.get_parameter_value_as_string(
            self._processor_id, self.gain_id
        )
